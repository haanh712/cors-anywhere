...
function withCORS(headers, request) {
  /* Allow cookies behind flag (bare minimum) */
  if(!request.corsAnywhereRequestState.enableCookies){
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  // Strip cookies
    if(req.corsAnywhereRequestState.enableCookies){
      var pathRegex = /path="((\/|\w|\?|\=|\:|\%|\.|\&|\#)*?)"/gm
    function updateCookies(cookies){
      var cookiePath = pathRegex.test(cookies)[0] || '/';
      return cookies.replace(pathRegex, "path="+ proxyReq.host + cookiePath +'"')
    }
    if(typeof proxyRes.headers['set-cookie'] === "string"){
      proxyRes.headers['set-cookie'] = updateCookies(proxyRes.headers['set-cookie'])
    }
    if(typeof proxyRes.headers['set-cookie2'] === "string"){
      proxyRes.headers['set-cookie2'] = updateCookies(proxyRes.headers['set-cookie2'])
    }
  }else{
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    req.corsAnywhereRequestState = {
      ...
      enableCookies: corsAnywhere.enableCookies
    };
